...
- [`sources.rs`](./src/sources.rs) — `Sources`: the aggregator that merges official repos and the AUR behind one `search`/`info` surface, and orchestrates `pacman.rs` + `alpm.rs` for `install`/`remove`. This is what the CLI/TUI should actually call, not the backends directly.
...
**Not yet implemented:** AUR building (`makepkg`, PKGBUILD fetch, build sandboxing), `upgrade`. See the root [`SPEC.md`](../../SPEC.md) Part X for the planned integration strategy, and [`../../dev/README.md`](../../dev/README.md) for how mutating commands are tested without touching the host.
